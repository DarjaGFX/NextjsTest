				...
				<AppBar dir="rtl" position="fixed" open={open}>
					...
					>
						...
						<List>
							...
							if (li.adminView && !auth.isAdmin) return;
							if (li.loginRequired && !auth.user) return;
							return (<ListItem  key={li.text} disablePadding>
								<ListItemButton  onClick={() => {handleDrawerButtonClick(li.text)}}>
									<ListItemIcon>
										<li.icon />
									</ListItemIcon>
									<ListItemText className='flex justify-end' primary={li.text} />
								</ListItemButton>
							</ListItem>
						)})}
						</List>
